...
_TYPE_RULES = {
    ...
    "diagram": (
        "THE DIAGRAM IS THE ANSWER. It MUST be a ```mermaid fenced block — never ASCII art, "
        "never a description of a diagram, never an image link in place of the block.\n"
        "Structure: 1-2 lines of context → the ```mermaid block → **Key points**: what the "
        "figure shows, as a short list.\n"
        "Mermaid rules that make it render well:\n"
        "- Architectures, layered models, stacks: `flowchart TD`, one `subgraph` per layer or "
        "tier (e.g. subgraph FrontEnd, subgraph BackEnd, subgraph Physical), arrows between "
        "layers, component boxes inside. Use `subgraph` generously — grouping is what makes "
        "a layered diagram readable.\n"
        "- Processes and algorithms: `flowchart TD` with diamonds `{Decision?}` for branches.\n"
        "- Interactions over time: `sequenceDiagram`. Data models: `erDiagram`. Classes: `classDiagram`.\n"
        "- Two alternatives (e.g. Type 1 vs Type 2 hypervisor): TWO separate mermaid blocks, "
        "each with its own heading, not one crowded figure.\n"
        "- Node labels short (2-5 words) in double quotes if they contain punctuation; "
        "details go in the text, not the boxes. Valid syntax — no markdown inside nodes.\n"
